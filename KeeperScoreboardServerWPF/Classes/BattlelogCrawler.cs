...
namespace KeeperScoreboardServerWPF.Classes
{
    public class BattlelogCrawler
    {
        List<Structs.ServerInfo> activePrivateServers = new List<Classes.Structs.ServerInfo>();
        public BattlelogCrawler()
        {
            MainWindow.activeServers = 0;
            string data;
                        string url =
                "http://battlelog.battlefield.com/bf4/servers/pc/?filtered=1&expand=1&settings=&useLocation=1&useAdvanced=1&gameexpansions=-1&gamemodes=137438953472&slots=16&slots=1&slots=2&q=&gameexpansions=-1&gameexpansions=-1&gameexpansions=-1&gameexpansions=-1&gameexpansions=-1&mapRotation=-1&modeRotation=-1&password=-1&osls=-1&vvsa=-1&vffi=-1&vaba=-1&vkca=-1&v3ca=-1&v3sp=-1&vmsp=-1&vrhe=-1&vhud=-1&vmin=-1&vnta=-1&vbdm-min=1&vbdm-max=300&vprt-min=1&vprt-max=300&vshe-min=1&vshe-max=300&vtkk-min=1&vtkk-max=99&vnit-min=30&vnit-max=86400&vtkc-min=1&vtkc-max=99&vvsd-min=0&vvsd-max=500&vgmc-min=0&vgmc-max=500";
            ...
            if(serverGuidList != null)
            {
                ...
                MainWindow.activePrivateServersTimeline.Add(new Structs.ServerInfoByTime
                {
                    time = DateTime.Now,
                    info = activePrivateServers
                });
            }
            Console.Write("");
        }
