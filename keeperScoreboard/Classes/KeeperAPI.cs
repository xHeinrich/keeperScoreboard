...
namespace keeperScoreboard.Classes
{
    class KeeperAPI
    {
        ...
        public CustomSnapshotRoot getKeeperInfo(string guid)
        {
            string keeperData = "";
            ...
            try
            {
                ...
                {
                    keeperData = wc.DownloadString(keeperURL + guid);
                }
            }catch(Exception)
            {
                Classes.Logger.addLog("The server crashed or was never available: " + guid);
            }
        ...
        {
            var GetNumPlayersOnServerString = 
